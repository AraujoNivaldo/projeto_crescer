...
class _telaAcompanhamento extends State<telaAcompanhamento> {
  ...
  ListTile _tile(String title, String subtitle, IconData icon, Measure medida) => ListTile(
        title: Text(title,
            style: TextStyle(
        ...
        onTap: () {
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                          builder: (context) => telaEditMedidasData(
                              crianca, medida)),
                    ).then((value) { setState(() {});});
                  }
      );
  ...
  ListView measureListView(context, List<Measure> data){
    ...
    shrinkWrap: true,
      ...
      itemBuilder: (context, index){
       return _tile(data[index].measureDate,"A: "+ data[index].measureHeight + "  P: " + data[index].measureWeight, Icons.edit, data[index]);
      });
  }

  @override
  Widget build(BuildContext context) {
    CollectionReference listaMedidas = FirebaseFirestore.instance.collection('measures');
    return Scaffold(
      appBar: AppBar(
        ...
        child: Center(
          child: Column(
            ...
            children: <Widget>[
              ...
              future: listaMedidas.where('childId', isEqualTo: crianca.id).get(),
              builder: (context, snapshot){
                if(snapshot.hasData){
                  ...
                  snapshot.data.docs.forEach((element) { 
                    ...
                    measurePerimeter = element.get('measurePerimeter');
                    measureIMC = element.get('measureIMC');
                    measureWeekDiff = element.get('measureWeekDiff');
                    measureMonthDiff = element.get('measureMonthDiff');
                    ...
                    newList.add(medidaNova);
                          ...
                          newList.sort((a, b) {
  return a.measureDate.toString().toLowerCase().compareTo(b.measureDate.toString().toLowerCase());
}); 
